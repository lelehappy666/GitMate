...
private final class SequencedGitHubAPI: GitHubAPI, @unchecked Sendable {
    ...
    func repositoryPage(
        ...
        let repositoryList = try await repositories(token: token)
        let normalizedPage = max(page, 1)
        let normalizedPerPage = max(perPage, 1)
        let start = (normalizedPage - 1) * normalizedPerPage
        let pageRepositories: [Repository]
    ...
    @unchecked Sendable
{
    ...
    func sync(
        ...
        lock.lock()
        recordedRepositoryIDs = repositories
            .filter { selectedRepositoryIDs.contains($0.id) }
            .map(\.id)
        lock.unlock()
...
private func makeViewModel(
    ...
    destinationStore: InMemorySyncDestinationStore? = nil,
    ...
    let dependencies = OnboardingDependencies(
        ...
        syncDestinationStore: destinationStore
            ?? InMemorySyncDestinationStore(destination: syncDestination),
